...
UNDEFINED_MACROS = ["glib2_gsettings_schema_requires", "openmpi_requires", "sysusers_requires"]
...
class SpecTags:
    ...
    def __init__(self, prj: str, pkg: str, cache_dir: Path):
        ...
        if int(obs_mdata_cache.get("revision", "-1")) >= int(obs_rev["rev"]):
            ...
            multi = osc.core.makeurl(
                self.API_URL, ["source", prj, pkg, "_multibuild"], query={"expand": 1}
            )
            ...
            if is_multi_flavoured:
                ...
                with NamedTemporaryFile(mode="w", suffix=".spec", dir=cache_dir) as pkg_spec:
                    ...
                    rpmspec_cmdline = (
                        f'{RPMSPEC_BIN.stdout.strip()} --srpm -q --qf "%{{url}} %{{version}}" '
                    )
                    rpmspec_cmdline += " ".join(
                        [f'--define="{macro} %nil"' for macro in UNDEFINED_MACROS]
                    )
                    rpmspec_cmdline += f" {pkg_spec.name}"
                    try:
                        proc = run(
                            rpmspec_cmdline, shell=True, capture_output=True, text=True, check=True
                        )
                        self.url, self.ver = proc.stdout.split()
                        spec_parse = run(
                            ["/usr/bin/rpmspec", "-P", pkg_spec.name],
                            capture_output=True,
                            text=True,
                            check=True,
                        )
                        ...
                        self.src_url = (
                            (self.RE_SRC0.search(spec_exp) or re.Match()).group().split()[-1]
                        )
