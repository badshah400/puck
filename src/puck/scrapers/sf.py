...
def sfLastVer(sfproj, srcf, oldver):
    ...
    sf_data = requests.get(
        f"https://sourceforge.net/projects/{sfproj}/best_release.json",
        headers=headers,
        timeout=30,
...
if __name__ == "__main__":
    ...
    for prj, pkg in f.List():
        ...
        if not sfprj:
            errs.Append(r"{:s}/{:s}: Source does not point to sourceforge URL".format(prj, pkg))
            continue
        ...
        try:
            ...
            errs.Append(r"{:s}/{:s}: Invalid package versiion {:s}".format(prj, pkg, uver))
            continue

        statusmap = {"specVer": specVer, "upsVer": parse(uver), "reqs": None, "update": False}
